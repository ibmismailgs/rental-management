...
@section('title', 'Edit Rent Collect')
@section('content')
@push('head')
    ...
    <div class="row">
        ...
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <form enctype="multipart/form-data" action="{{ route('rent-collect.update', $data->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="rent_info_id" value="{{ $data->rent_info_id }}">
                                        <input type="hidden" name="owner_info_id" value="{{ $data->owners->id }}">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="month" id="rental_month" name="rental_month" value="{{ $data->rental_month }}"class="form-control @error('rental_month') is-invalid @enderror" placeholder="Enter month" required>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="receive_amount" name="receive_amount" value="{{ json_decode($data->amount) ?? 0 }}"class="form-control @error('address') is-invalid @enderror" placeholder="Enter receive amount" required @if ($due == 0) readonly @endif>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="due_amount" name="due_amount" value="{{ $due ?? 0 }}" class="form-control" readonly>
                                    </div>
                                </div>
                                            ...
                                            <option value="">Select account</option>
                                                @foreach ($mobilebanks as $key => $mobilebank)
                                                    @if($mobilebank->owner_info_id == $data->owner_info_id)
                                                        <option value="{{ $bank->id }}" @if($mobilebank->id == $data->mobile_banking_id) selected @endif>{{ $mobilebank->mobileBankings->name }} || {{ $mobilebank->owners->owner_name }} || {{ $mobilebank->mobile_no }}</option>
                                                    @endif
                                                @endforeach
                                        </select>
